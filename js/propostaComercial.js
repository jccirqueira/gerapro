...
const PropostaComercialModule = {
    init() {
        ...
        window.app.propostaComercial = {
            ...
            handleUnidadeChange: this.handleUnidadeChange.bind(this)
        };
    ...
    _captureFormData(form, current = {}) {
        ...
        const result = Object.assign({}, current, {
            ...
            unidade_cliente: get('unidade_cliente'),
...
${store.canEdit() ? `                        <button class="btn-icon" onclick="app.propostaComercial.edit('${p.id}')" title="Editar">
        ...
        return `
            ...
            <div style="padding: 40px; max-width: 1200px; margin: 0 auto;">
                ...
                <div class="form-group" style="margin-top: 16px;">
                    ...
                    <select name="unidade_cliente" id="pc_unidade_cliente" class="form-control" onchange="app.propostaComercial.handleUnidadeChange(this.value)">

                        <option value="">Selecione uma unidade...</option>

                        ${data.unidade_cliente ? `<option value="${this._escapeHtml(data.unidade_cliente)}" selected>${this._escapeHtml(data.unidade_cliente)}</option>` : ''}
    ...
    updateContactDropdown(clientName, selectedContact = null) {
        ...
        if (unidadeSel) {
            if (client) {
                ...
                const savedUnidade = store.getState().activeCommercialProposal?.unidade_cliente || window.app.currentPtc?.unidade_cliente || window.app.currentPtcInfo?.autpro?.codigoUnidade || '';
    ...
    handleContactSelection(contactName) {
        ...
        if (isAUTPRO) {
            const codigoUnidade = document.getElementById('pc_unidade_cliente')?.value;
            if (codigoUnidade) {
                const slug = (s) => (s || '').toString().toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^a-z0-9]/g, '');
    ...
    handleUnidadeChange(codigoUnidade) {
        ...
        const data = store.getState().activeCommercialProposal;
        if (data) {
            data.unidade_cliente = codigoUnidade;
            store.setState({ activeCommercialProposal: data });
        }
        if (this._proposalData) {
            this._proposalData.unidade_cliente = codigoUnidade;
        }

        const aosCuidados = document.getElementById('pc_aos_cuidados');
        if (aosCuidados) {
            if (contacts.length > 0) {
    ...
    save() {
        ...
        const proposta = {
            ...
            cliente: finalClient,
            unidade_cliente: formData.get('unidade_cliente') || '',
